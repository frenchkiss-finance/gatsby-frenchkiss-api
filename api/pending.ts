...
const pending = async (pid: number, address: string) => {
  const chef = getContract(chefABI, "");
  const pending = await chef.methods.pendingKiss(pid, address).call();
...
export default async (req: NowRequest, res: NowResponse): Promise<void> => {
  const { address = "0x0b5E8214562dd59e204717CEe782904EC4B12B6D", pid = "1" } = req.query;
